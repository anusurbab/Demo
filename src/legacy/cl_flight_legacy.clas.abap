CLASS cl_flight_legacy DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE
  GLOBAL FRIENDS cl_flight_data_generator.

  PUBLIC SECTION.
    INTERFACES if_flight_legacy.
    ...
    CLASS-METHODS: get_instance RETURNING VALUE(ro_instance) TYPE REF TO cl_flight_legacy.
    ...
    METHODS create_travel IMPORTING is_travel             TYPE s_travel_in
                                    it_booking            TYPE t_booking_in OPTIONAL
                                    it_booking_supplement TYPE t_booking_supplement_in OPTIONAL
                          EXPORTING es_travel             TYPE travel
                                    et_booking            TYPE t_booking
                                    et_booking_supplement TYPE t_booking_supplement
                                    et_messages           TYPE if_flight_legacy=>tt_if_t100_message.
    METHODS update_travel IMPORTING is_travel              TYPE s_travel_in
                                    is_travelx             TYPE s_travel_inx
                                    it_booking             TYPE t_booking_in OPTIONAL
                                    it_bookingx            TYPE t_booking_inx OPTIONAL
                                    it_booking_supplement  TYPE t_booking_supplement_in OPTIONAL
                                    it_booking_supplementx TYPE t_booking_supplement_inx OPTIONAL
                          EXPORTING es_travel              TYPE travel
                                    et_booking             TYPE t_booking
                                    et_booking_supplement  TYPE t_booking_supplement
                                    et_messages            TYPE if_flight_legacy=>tt_if_t100_message.
    METHODS delete_travel IMPORTING iv_travel_id TYPE travel_id
                          EXPORTING et_messages  TYPE if_flight_legacy=>tt_if_t100_message.
    METHODS get_travel IMPORTING iv_travel_id           TYPE travel_id
    ...
    METHODS save.
    METHODS initialize.
    METHODS convert_messages IMPORTING it_messages TYPE if_flight_legacy=>tt_if_t100_message
                             EXPORTING et_messages TYPE t_message.
  PROTECTED SECTION.
  PRIVATE SECTION.
    CLASS-DATA go_instance TYPE REF TO cl_flight_legacy.
    ...
    METHODS _determine EXPORTING et_messages           TYPE if_flight_legacy=>tt_if_t100_message
                       CHANGING  cs_travel             TYPE travel
                                 ct_booking            TYPE t_booking
                                 ct_booking_supplement TYPE t_booking_supplement.
    METHODS _determine_travel_total_price CHANGING cs_travel             TYPE travel
                                                   ct_booking            TYPE t_booking
                                                   ct_booking_supplement TYPE t_booking_supplement
                                                   ct_messages           TYPE if_flight_legacy=>tt_if_t100_message ##NEEDED.
    METHODS _convert_currency IMPORTING iv_currency_code_source TYPE currency_code
                                        iv_currency_code_target TYPE currency_code
                                        iv_amount               TYPE total_price
                              RETURNING VALUE(rv_amount)        TYPE total_price.
ENDCLASS.



CLASS CL_FLIGHT_LEGACY IMPLEMENTATION.
  ...
  METHOD convert_messages.
    ...
    LOOP AT it_messages INTO DATA(lr_error) ##INTO_OK.
      ...
      IF lr_error IS INSTANCE OF cx_flight_legacy.
        DATA(lx) = CAST cx_flight_legacy( lr_error ).
  ...
  METHOD create_travel.
    ...
    lcl_travel_buffer=>get_instance( )->cud_prep( EXPORTING it_travel   = VALUE #( ( CORRESPONDING #( is_travel ) ) )
                                                            it_travelx  = VALUE #( ( travel_id = is_travel-travel_id  action_code = if_flight_legacy=>action_code-create ) )
    ...
    " Bookings
    IF et_messages IS INITIAL.
      DATA lt_booking  TYPE t_booking.
      DATA lt_bookingx TYPE t_bookingx.
      LOOP AT it_booking INTO DATA(ls_booking_in).
        ...
        INSERT VALUE #( travel_id = ls_booking-travel_id  booking_id = ls_booking-booking_id  action_code = if_flight_legacy=>action_code-create ) INTO TABLE lt_bookingx.
      ENDLOOP.
    ...
    " Booking Supplements
    IF et_messages IS INITIAL.
      DATA lt_booking_supplement  TYPE t_booking_supplement.
      DATA lt_booking_supplementx TYPE t_booking_supplementx.
      LOOP AT it_booking_supplement INTO DATA(ls_booking_supplement_in).
        DATA ls_booking_supplement TYPE book_suppl.
        ...
        INSERT VALUE #( travel_id             = ls_booking_supplement-travel_id
                        ...
                        action_code           = if_flight_legacy=>action_code-create ) INTO TABLE lt_booking_supplementx.
      ENDLOOP.
      IF et_messages IS INITIAL.
  ...
  METHOD delete_travel.
    ...
    IF et_messages IS INITIAL.
      lcl_booking_supplement_buffer=>get_instance( )->cud_prep( EXPORTING it_booking_supplement  = CORRESPONDING #( lt_booking_supplement MAPPING travel_id             = travel_id
                                                                          ...
                                                                          it_booking_supplementx = VALUE #( FOR ls_bs IN lt_booking_supplement ( action_code           = if_flight_legacy=>action_code-delete
    ...
    IF et_messages IS INITIAL.
      lcl_booking_buffer=>get_instance( )->cud_prep( EXPORTING it_booking         = CORRESPONDING #( lt_booking MAPPING travel_id = travel_id  booking_id = booking_id EXCEPT * )
                                                               it_bookingx        = VALUE #( FOR ls_b IN lt_booking ( action_code = if_flight_legacy=>action_code-delete  travel_id = ls_b-travel_id  booking_id = ls_b-booking_id ) )
                                                               iv_no_delete_check = abap_true " No existence check required
    ...
    IF et_messages IS INITIAL.
      lcl_travel_buffer=>get_instance( )->cud_prep( EXPORTING it_travel          = VALUE #( ( travel_id = iv_travel_id ) )
                                                              it_travelx         = VALUE #( ( travel_id = iv_travel_id  action_code = if_flight_legacy=>action_code-delete ) )
                                                              iv_no_delete_check = abap_true " No existence check required
                                                    IMPORTING et_messages        = lt_messages ).
      APPEND LINES OF lt_messages TO et_messages.
    ENDIF.
...
*     CALL FUNCTION 'ENQUEUE_ETRAVEL'
*       EXCEPTIONS
*         foreign_lock   = 1
*         system_failure = 2
*         OTHERS         = 3.
*     IF sy-subrc <> 0.
*       RAISE EXCEPTION TYPE cx_flight_legacy
*         EXPORTING
*           textid = cx_flight_legacy=>travel_lock.
*     ENDIF.
*   ELSE.
*     CALL FUNCTION 'DEQUEUE_ETRAVEL'.
*   ENDIF.
  ENDMETHOD.
  ...
  METHOD update_travel.
    ...
    IF is_travel-travel_id IS INITIAL.
      ...
      RETURN.
    ENDIF.
    DATA ls_travelx TYPE s_travelx.
    ls_travelx = CORRESPONDING #( is_travelx ).
    ls_travelx-action_code = if_flight_legacy=>action_code-update.
    lcl_travel_buffer=>get_instance( )->cud_prep( EXPORTING it_travel   = VALUE #( ( CORRESPONDING #( is_travel ) ) )
    ...
    IF et_messages IS INITIAL.
      ...
      LOOP AT it_bookingx INTO DATA(ls_booking_inx).
        DATA ls_bookingx TYPE s_bookingx.
    ...
    IF et_messages IS INITIAL.
      ...
      IF et_messages IS INITIAL.
        LOOP AT it_booking_supplementx INTO DATA(ls_booking_supplement_inx).
          DATA ls_booking_supplementx TYPE s_booking_supplementx.
    ...
    IF    et_messages IS INITIAL
      ...
      AND line_exists( lt_bookingx[ action_code = CONV action_code( if_flight_legacy=>action_code-delete ) ] ).
      ...
      LOOP AT lt_booking_supplement_del ASSIGNING FIELD-SYMBOL(<s_booking_supplement_del>).
        READ TABLE lt_bookingx TRANSPORTING NO FIELDS WITH KEY action_code = CONV action_code( if_flight_legacy=>action_code-delete )
      ...
      lcl_booking_supplement_buffer=>get_instance( )->cud_prep( EXPORTING it_booking_supplement  = CORRESPONDING #( lt_booking_supplement_del MAPPING travel_id             = travel_id
                                                                                                                                                      booking_id            = booking_id
                                                                                                                                                      booking_supplement_id = booking_supplement_id EXCEPT * )
                                                                          it_booking_supplementx = VALUE #( FOR ls_bs IN lt_booking_supplement_del ( action_code           = if_flight_legacy=>action_code-delete
                                                                                                                                                     travel_id             = ls_bs-travel_id
                                                                                                                                                     booking_id            = ls_bs-booking_id
                                                                                                                                                     booking_supplement_id = ls_bs-booking_supplement_id ) )
                                                                          iv_no_delete_check     = abap_true " No existence check required
      ...
      IF et_messages IS INITIAL.
        ...
        LOOP AT et_booking ASSIGNING FIELD-SYMBOL(<s_booking>).
          LOOP AT it_bookingx TRANSPORTING NO FIELDS WHERE booking_id = <s_booking>-booking_id
            AND ( action_code = CONV action_code( if_flight_legacy=>action_code-create ) OR action_code = CONV action_code( if_flight_legacy=>action_code-update ) ).
            EXIT.
          ENDLOOP.
          IF sy-subrc <> 0.
            DELETE et_booking.
  ...
  METHOD _determine_travel_total_price.
    ...
    IF lv_currency_code_target IS NOT INITIAL.
      ...
      lcl_travel_buffer=>get_instance( )->cud_prep( EXPORTING it_travel   = VALUE #( ( travel_id = cs_travel-travel_id  total_price = cs_travel-total_price  currency_code = cs_travel-currency_code ) )
                                                              it_travelx  = VALUE #( ( action_code = if_flight_legacy=>action_code-update  travel_id = cs_travel-travel_id  total_price = abap_true  currency_code = abap_true ) )
                                                    IMPORTING et_messages = DATA(lt_messages) ).
