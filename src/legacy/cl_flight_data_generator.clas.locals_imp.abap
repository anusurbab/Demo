...
CLASS lcl_agency_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_agency TYPE STANDARD TABLE OF agency WITH KEY agency_id.
...
CLASS lcl_airport_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_airport TYPE STANDARD TABLE OF airport WITH KEY airport_id.
    CLASS-METHODS: get_data
...
CLASS lcl_airport_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM airport.                           "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data(  ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT airport FROM TABLE @lt_data.
  ...
  METHOD get_data.
    rt_data = VALUE tt_airport(
          ...
          ( airport_id = 'SXF'    name = 'Berlin SchÃ¶nefeld Airport'              city = 'Berlin'                            country = 'DE' )
          ...
          ( airport_id = 'ZRH'    name = 'ZÃ¼rich Airport'                         city = 'Zurich'                            country = 'CH' )
          ( airport_id = 'RTM'    name = 'Rotterdam The Hague Airport'            city = 'Rotterdam'                         country = 'NL' )
          ( airport_id = 'FCO'    name = 'Leonardo da Vinciâ€“Fiumicino Airport'    city = 'Rome'                              country = 'IT' )
          ...
          ( airport_id = 'MAD'    name = 'Adolfo SuÃ¡rez Madridâ€“Barajas Airport'   city = 'Madrid'                            country = 'ES' )
          ...
          ( airport_id = 'YOW'    name = 'Ottawa Macdonaldâ€“Cartier Int. Airport'  city = 'Ottawa, Ontario'                   country = 'CA' )
          ( airport_id = 'ACA'    name = 'General Juan N. Ã�lvarez Int. Airport'   city = 'Acapulco, Guerrero'                country = 'MX' )
          ( airport_id = 'GIG'    name = 'Rio de Janeiroâ€“GaleÃ£o Int. Airport'     city = 'Rio de Janeiro'                    country = 'BR' )
          ( airport_id = 'HAV'    name = 'JosÃ© MartÃ­ International Airport'       city = 'Havana'                            country = 'CU' )
...
CLASS lcl_carrier_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_carrier TYPE STANDARD TABLE OF carrier WITH KEY carrier_id.
    CLASS-METHODS: get_data
      RETURNING
...
CLASS lcl_carrier_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM carrier.                           "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data( ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT carrier FROM TABLE @lt_data.
...
CLASS lcl_connection_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_connection TYPE STANDARD TABLE OF connection WITH KEY carrier_id connection_id.
...
CLASS lcl_connection_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ). ENDIF.
    DELETE FROM connection.                        "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ).  ENDIF.
    INSERT connection FROM TABLE @lt_data.
...
CLASS cl_flight_data_generator DEFINITION LOCAL FRIENDS lcl_flight_data_generator.
CLASS lcl_flight_data_generator DEFINITION CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_flights TYPE STANDARD TABLE OF flight
  ...
  PRIVATE SECTION.
    TYPES: BEGIN OF ty_plane_type,
             id            TYPE plane_type_id,
             seats_max     TYPE plane_seats_max,
           ...
           BEGIN OF ty_flight_info,
             ...
             long_distance  TYPE abap_bool,
             seats_max      TYPE plane_seats_max,
             seats_occupied TYPE plane_seats_occupied,
             price          TYPE flight_price,
           END OF ty_flight_info,
           BEGIN OF ty_connection_recurrency,
             id         TYPE connection_id,
             recurrency TYPE STANDARD TABLE OF flight_date WITH EMPTY KEY,
    ...
    CLASS-METHODS: build_dependent_content,
      get_flight_info
        IMPORTING
          iv_connection_id     TYPE connection_id
        RETURNING
...
CLASS lcl_flight_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM flight.                            "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ).  ENDIF.
    INSERT flight FROM TABLE @gt_flights.
    ...
    LOOP AT gt_connections INTO DATA(ls_connection).
      LOOP AT gt_connection_recurrency[ id = ls_connection-connection_id ]-recurrency INTO DATA(lv_flight_date).
        ...
        APPEND VALUE flight(
                  ...
                  currency_code  = VALUE flight-currency_code(
  ...
  METHOD get_flight_info.
    ...
    rs_plane_info = VALUE ty_flight_info(
        ...
        price          = cl_flight_data_generator=>calculate_flight_price(
  ...
  METHOD build_connection_recurrency.
    ...
    flight_date_max = calc_next_monday( CONV flight_date( lv_datum + 217 ) ).

    " flight_date_min is a Monday 5 months in the past
    flight_date_min = calc_next_monday( CONV flight_date( lv_datum - 148 ) ).
...
CLASS lcl_customer_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_last_name,
           ...
           END OF ty_last_name.
    TYPES: tt_customer  TYPE STANDARD TABLE OF customer WITH KEY customer_id,
  ...
  PRIVATE SECTION.
    TYPES:
      ...
      BEGIN OF ty_city,
        ...
        city        TYPE city,
      END OF ty_city,
      tt_street_per_country TYPE STANDARD TABLE OF street WITH EMPTY KEY,
      BEGIN OF ty_street,
        country TYPE land1,
...
CLASS lcl_customer_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM customer.                          "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data( ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT customer FROM TABLE @lt_data.
    ...
    LOOP AT build_last_names( ) INTO DATA(ls_last_name).
      DO lo_random_counter->get_next( ) TIMES.
        ...
        APPEND VALUE ty_name(
            ...
            title      = COND title( WHEN ls_first_name-gender = 'M' THEN 'Mr.'
  ...
  METHOD build_city.
    rt_data = VALUE tt_city(
              ...
              ( country = 'FR' postal_code =    '78140' city = 'VÃ©lizy')
              ( country = 'CH' postal_code =     '1211' city = 'GenÃ¨ve')
...
CLASS lcl_supplement_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_supplement      TYPE STANDARD TABLE OF supplement WITH KEY supplement_id,
           tt_supplement_text TYPE STANDARD TABLE OF suppl_text WITH KEY supplement_id.

    " Merged types
    TYPES BEGIN OF ty_supplement_complete.
    INCLUDE TYPE supplement.
...
CLASS lcl_supplement_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM supplement.                        "#EC CI_NOWHERE
    DELETE FROM suppl_text.                        "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data( ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT supplement FROM TABLE @( CORRESPONDING tt_supplement( lt_data ) ).
    INSERT suppl_text FROM TABLE @( CORRESPONDING tt_supplement_text( lt_data ) ).
...
CLASS cl_flight_data_generator DEFINITION LOCAL FRIENDS lcl_travel_data_generator.
CLASS lcl_travel_data_generator DEFINITION CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES: lif_data_generator.

    TYPES: tt_travel              TYPE STANDARD TABLE OF travel     WITH KEY travel_id,
           tt_bookings            TYPE STANDARD TABLE OF booking    WITH KEY travel_id booking_id,
           tt_booking_supplements TYPE STANDARD TABLE OF book_suppl WITH KEY travel_id booking_id booking_supplement_id.

    " Build nested tables
    TYPES BEGIN OF ty_booking_complete.
    INCLUDE TYPE booking.
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS:
      ...
      build_booking
        IMPORTING
          iv_travel_id       TYPE booking-travel_id
        RETURNING
      ...
      find_next_fitting_flight
        IMPORTING
          ...
          is_flight_previous TYPE flight OPTIONAL
        RETURNING
          VALUE(rs_flight)   TYPE flight,
      generate_booking_supplements
        IMPORTING
          iv_travel_id   TYPE booking-travel_id
          iv_booking_id  TYPE booking-booking_id
        RETURNING
      ...
      generate_description
        ...
        RETURNING
          VALUE(rv_description) TYPE travel-description,
      ...
      calculate_booking_fee
        IMPORTING
          it_bookings           TYPE lcl_travel_data_generator=>tt_booking_complete
        RETURNING
          VALUE(rv_booking_fee) TYPE travel-booking_fee,
      generate_travel_customer_id
        IMPORTING
          it_bookings           TYPE lcl_travel_data_generator=>tt_booking_complete
        RETURNING
          VALUE(rv_customer_id) TYPE travel-customer_id,
      set_today,
...
CLASS lcl_travel_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    ...
    LOOP AT lt_data INTO DATA(ls_travel).
      APPEND CORRESPONDING travel( ls_travel ) TO lt_travels.
      LOOP AT ls_travel-bookings INTO DATA(ls_booking).
        APPEND CORRESPONDING booking( ls_booking ) TO lt_bookings.
    ...
    INSERT travel FROM TABLE @lt_travels.
    IF out IS BOUND.  out->write( '--> Insert Booking Content' ).  ENDIF.
    INSERT booking FROM TABLE @lt_bookings.
    IF out IS BOUND.  out->write( '--> Insert Booking Supplement Content' ).  ENDIF.
    INSERT book_suppl FROM TABLE @lt_booking_supplements.
  ...
  METHOD get_data.
    DATA: lv_travel_id TYPE booking-travel_id.
    ...
    WHILE lt_bookings IS NOT INITIAL.
      ...
      APPEND VALUE ty_travel_complete(
          ...
          total_price   = lv_booking_fee + REDUCE travel-total_price( INIT sum = 0
                                                          FOR booking IN lt_bookings
                                                          NEXT
                                                           sum = sum
                                                                 + booking-flight_price
                                                                 + REDUCE flight_price( INIT sum_supplement = 0
          ...
          status        = SWITCH travel-status( go_ran_status_description->get_next( )
                                          WHEN 1 OR 2 THEN if_flight_legacy=>travel_status-new
                                          WHEN 3      THEN if_flight_legacy=>travel_status-booked
                                          WHEN 4      THEN if_flight_legacy=>travel_status-planned  )
  ...
  METHOD build_booking.
    TYPES: tt_customer_id TYPE TABLE OF customer-customer_id WITH EMPTY KEY.
    DATA: lv_booking_id      TYPE booking-booking_id,
    ...
    DO lv_trip_length TIMES.
      ...
      DATA(lv_booking_date) = CONV booking-booking_date( <flight>-flight_date - go_ran_booking_date->get_next( ) ).

      DATA(lv_price) = cl_flight_data_generator=>calculate_flight_price(
      ...
      FOR i = 1  THEN i + 1  WHILE i <= lines( lt_customer_id )
         (
            ...
            booking_supplements = generate_booking_supplements( iv_travel_id = iv_travel_id   iv_booking_id = CONV booking-booking_id( lv_booking_id + i ) )
          )
      ) TO rt_bookings.
  ...
  METHOD find_next_fitting_flight.
    ...
    IF is_flight_previous IS SUPPLIED.
      DATA(lv_connection_id_new) = VALUE connection-connection_id( gt_connections[
  ...
  METHOD generate_description.
    TYPES: tt_customers TYPE SORTED TABLE OF customer WITH UNIQUE KEY customer_id.
    rv_description = SWITCH travel-description(
                               go_ran_travel_description->get_next( )
                                  WHEN 1 THEN `Business Trip for ` &&
                                                REDUCE travel-description(
                                                               ...
                                                               NEXT s =  s && gt_customer[ customer_id = traveler-customer_id ]-first_name
                                                                           && COND travel-description( WHEN i < lines( it_bookings ) THEN `, ` )
                                                                    i = i + 1 )
  ...
  METHOD generate_travel_customer_id.
    rv_customer_id = COND travel-customer_id(
