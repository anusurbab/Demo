CLASS tc_flight_travel_api DEFINITION
  PUBLIC
  ABSTRACT
  CREATE PUBLIC
  FOR TESTING
...
CLASS TC_FLIGHT_TRAVEL_API IMPLEMENTATION.
  ...
  METHOD cuerd_travel.
    DATA ls_travel_in  TYPE s_travel_in.
    DATA ls_travel_inx TYPE s_travel_inx.
    ...
    " DB Check
    SELECT SINGLE agency_id, customer_id, description FROM travel WHERE travel_id = @lv_travel_id INTO ( @DATA(lv_agency_id), @DATA(lv_customer_id), @DATA(lv_description) ).
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_travel-status       exp = CONV travel_status( if_flight_legacy=>travel_status-new ) ).
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_travel-status       exp = CONV travel_status( if_flight_legacy=>travel_status-booked ) ).
